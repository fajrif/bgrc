class Admins::EventsController < Admins::BaseController
  ...
  def index
		if params[:search].blank?
			...
			criteria = Event.where("name ->> :key ILIKE :value", key: I18n.locale.to_s, value: "%#{params[:search]}%")
		end
  ...
  def params_event
    params.require(:event).permit(:image, :name, :short_description, :description, :start_date, :end_date, :featured, :sport_id, :capacity, images: [])
  end
